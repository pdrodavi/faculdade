...
public class ServletEditarAluno extends HttpServlet {
    ...
    protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            if (type.equalsIgnoreCase("editarAluno")) {

                Long id = Long.valueOf(req.getParameter("id"));
                user = new Usuario(id, nome, cpf, endereco, email, telefone, senha, perfil);
                dao.updateUsuario(user);
                
                redirect(resp, "pages/diretor/listaDeAlunos", "Aluno editado com sucesso!");
                
            } else {
                redirect(resp, "pages/diretor/listaDeProfessores", "Não foi possível editar o Aluno!");
            }
        } catch (Exception ex) {
            System.out.println("ERRO NO SERVLET DE EDITAR ALUNO" + ex);
        }
